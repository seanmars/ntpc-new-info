...
回傳最近一次成功從上游抓取的救援事件 GeoJSON 快照. 此 endpoint 不會主動觸發上游抓取, 僅讀取 in-memory cache.
...
繞過輪詢節奏, 同步觸發一次上游抓取. 成功後 snapshot 與排程輪詢路徑共享, detector (含 Discord 通知) 行為一致. 受 `ForceRefreshCooldownSeconds` 冷卻保護.

- **200 OK** - 抓取成功, body 結構與 `GET /api/rescue/latest` 200 相同
- **429 Too Many Requests** - 冷卻時間未到或另一次 refresh 進行中. 回應含 `Retry-After` header:

  ```json
  {
    "error": "manual refresh cooldown is active",
    "retryAfterSeconds": 12
  }
  ```

- **502 Bad Gateway** - 上游抓取失敗 (timeout, 非 2xx, deserialization 錯誤等), snapshot 維持不變:

  ```json
  {
    "error": "upstream fetch failed",
    "lastError": "...",
    "lastErrorAt": "2026-05-17T12:34:56Z"
  }
  ```
...
- `ForceRefreshCooldownSeconds`: `POST /api/rescue/refresh` 成功之間的最小間隔 (秒, 預設 15, 有效範圍 1-3600). 排程輪詢不受此限.
...
- Status bar 提供兩種更新按鈕:「重新整理」(重讀 backend cache) 與「強制更新」(呼叫 `POST /api/rescue/refresh` 觸發上游重抓). 強制更新受 backend cooldown 保護, 冷卻期間按鈕停用並顯示倒數
